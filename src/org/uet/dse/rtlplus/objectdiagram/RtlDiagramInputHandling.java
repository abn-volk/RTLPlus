 ...
 */
public final class RtlDiagramInputHandling
		implements MouseListener, MouseMotionListener, DropTargetListener, KeyListener {
	...
	protected void moveSelectedObjects(int dx, int dy) {
		...
		for (PlaceableNode sel : fNodeSelection) {
			if (sel instanceof ObjectNode) {
				if (((ObjectNode) sel).checkNewPositition(sel.getX() + dx, sel.getX() + sel.getWidth() + dx)) {
	...
	public void drop(DropTargetDropEvent dtde) {
		fDiagram.dropObjectFromModelBrowser(dtde);
	}
