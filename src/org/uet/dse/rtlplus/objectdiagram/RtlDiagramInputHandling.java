 ...
 */
public final class RtlDiagramInputHandling implements MouseListener,
                                                   MouseMotionListener, 
                                                   DropTargetListener,
                                                   KeyListener {
    ...
    public RtlDiagramInputHandling( Selection<PlaceableNode> nodeSelection, Selection<EdgeBase> edgeSelection, DiagramView diagram ) {
	...
	protected void moveSelectedObjects(int dx, int dy) {
		...
		for (PlaceableNode sel : fNodeSelection) {
			if (sel instanceof ObjectNode) {
				if (((ObjectNode) sel).checkNewPositition(sel.getX() + dx)) {
