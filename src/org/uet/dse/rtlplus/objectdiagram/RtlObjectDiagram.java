...
public class RtlObjectDiagram extends RtlDiagramView implements HighlightChangeListener, SortChangeListener {

	/**
	 * This class saves needed information about visible or hidden nodes and edges.
	 * When hiding or showing objects the data is moved between the fields
	 * {@link RtlObjectDiagram#visibleData} and {@link RtlObjectDiagram#hiddenData}.
	 * 
	 * @author Lars Hamann
	 */
	public static class RtlObjectDiagramData implements DiagramData {
		/**
		 * Map from object instances to object nodes. It also includes link objects.
		 */
		 ...
		 * @param hiddenData
		 */
		public void copyTo(RtlObjectDiagramData target) {
	...
	private RtlObjectDiagramData visibleData = new RtlObjectDiagramData();

	private RtlObjectDiagramData hiddenData = new RtlObjectDiagramData();
	 ...
	 * to a specific position when an object is created by drag & drop.
	 */
	private Point2D.Double nextNodePosition = new Point2D.Double();

	private ShowObjectPropertiesViewMouseListener showObjectPropertiesViewMouseListener = new ShowObjectPropertiesViewMouseListener();
	 ...
	 */
	RtlObjectDiagram(RtlObjectDiagramView newObjectDiagramView, Map<String, MRuleCollection.Side> classMap, PrintWriter log) {
		this(newObjectDiagramView, classMap, log, new ObjDiagramOptions());
	}

	RtlObjectDiagram(RtlObjectDiagramView newObjectDiagramView, Map<String, MRuleCollection.Side> classMap, PrintWriter log, ObjDiagramOptions options) {
		super(options, log);
	...
	public ObjectNode getObjectNode(MObject mObject){
    	ObjectNode node = this.visibleData.fObjectToNodeMap.get(mObject);
    	if (node == null)
    		return this.hiddenData.fObjectToNodeMap.get(mObject);
    	else return node;
    }
	...
	protected void showOrHideObjectNode(MObject obj, boolean show) {
		RtlObjectDiagramData source = (show ? hiddenData : visibleData);
		RtlObjectDiagramData target = (show ? visibleData : hiddenData);
	...
	protected void addBinaryLink(MLink link) {
		...
		if (link instanceof MLinkObject) {
			BinaryAssociationClassOrObject e = BinaryAssociationClassOrObject.create(
					...
					linkEnd1.associationEnd(), linkEnd2.associationEnd(), visibleData.fObjectToNodeMap.get((MLinkObject) link), this,
					link);
			...
			BinaryAssociationOrLinkEdge e = BinaryAssociationOrLinkEdge.create(node1, node2, linkEnd1.associationEnd(),
	...
	protected void addNAryLink(MLink link) {
		...
		if (link instanceof MLinkObject) {
			NAryAssociationClassOrObjectEdge e = NAryAssociationClassOrObjectEdge.create(node,
					visibleData.fObjectToNodeMap.get((MLinkObject) link), this, link.association(), true);
	...
	protected PopupMenuInfo unionOfPopUpMenu() {
		...
		if (!selectedObjects.isEmpty()) {
			popupMenu.insert(getAction("Hide " + selectedObjectsText, selectedObjectsSet), pos++);
			popupMenu.insert(getAction("Crop " + selectedObjectsText, getNoneSelectedNodes(selectedObjectsSet)), pos++);
			popupMenu.insert(new JSeparator(), pos++);
		}
	...
	protected void storePlacementInfos(PersistHelper helper, Element root, boolean visible) {
		RtlObjectDiagramData data = (visible ? visibleData : hiddenData);
